...
describe('Pump Builder', () => {
  it('return pump as a promise', done => {
    ...
    const pumpBuilder = createPump()
      ...
      .throughF((d) => Number.parseInt(d, 10))
      .onData((d) => {
  ...
  it('should compose pipeline function with type safety', done => {
    ...
    const f2: (s: string[]) => number[] = s => s.map(s0 => Number.parseInt(s0, 10));
  ...
  const numberStream = (start: number, end: number): Readable => arrayStream(_.range(start, end));
